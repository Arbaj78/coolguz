...
const SinglePost = () => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-6">
          ...
          <Link to="/blog" className="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors duration-200 shadow-lg hover:shadow-xl">
            Return to Blog
          </Link>
        </div>
      </div>
  ...
  const excerpt = (() => {
    try {
      ...
      const first = singlePost.body.find(b => b._type === 'block' && b.children && b.children.length);
      if (!first) return '';
      return first.children.map(c => c.text || '').join('').slice(0, 160);
    } catch {
      return '';
    }
  })();
  ...
  const jsonLd = {
    ...
    author: singlePost.authorName ? { "@type": "Person", name: singlePost.authorName } : undefined,
      ...
      <div className="relative">
        {singlePost.mainImage && singlePost.mainImage.asset?.url && (
          <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
            <img src={urlFor(singlePost.mainImage).width(1600).url()} alt={singlePost.mainImage.alt || "Blog cover"} className="w-full h-full object-cover" loading="eager" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
            <div className="absolute top-6 left-6 z-10">
              <Link to="/blog" className="inline-flex items-center px-4 py-2 bg-white/90 backdrop-blur-sm text-gray-800 font-medium rounded-full hover:bg-white transition-all duration-200 shadow-lg hover:shadow-xl">
                Back
              </Link>
            </div>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold text-white mb-4 leading-tight">{singlePost.title}</h1>
            {singlePost.publishedAt && <div className="flex items-center text-white/90"><Calendar size={20} className="mr-2" /><span>{formatDate(singlePost.publishedAt)}</span></div>}
          </div>
        </div>
      </div>
      ...
      <div className="relative -mt-20 z-10">
        <div className="max-w-4xl mx-auto px-6 md:px-8">
                    ...
                    types: {
                      image: ({ node }) => (
                        <figure className="my-8 rounded-xl overflow-hidden shadow-lg">
                          <img src={urlFor(node).width(800).url()} alt={node.alt || "Blog Image"} className="w-full h-auto" loading="lazy" />
                          {node.caption && <figcaption className="text-center text-sm text-gray-500 mt-2">{node.caption}</figcaption>}
                        </figure>
                      ),
                    },
                  }}
          ...
          <div className="mt-12 mb-16 text-center">
            <Link to="/blog" className="inline-flex items-center px-8 py-4 bg-gray-900 text-white font-semibold rounded-xl hover:bg-gray-800 transition-all duration-200 shadow-lg hover:shadow-xl">
