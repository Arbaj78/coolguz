...
const DomainsSection = () => {
  ...
  const handleCardClick = (domain) => {
    // progressive enhancement: navigate to link if present
    if (domain.link) window.location.href = domain.link;
  };
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-slate-900 via-slate-900 to-blue-900/10 relative overflow-hidden">
      <SEO {...seo["/Domain"]} url={`${SITE.domain}/Domain`} />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(0,198,255,0.06),transparent_50%)]" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_80%,rgba(123,97,255,0.06),transparent_50%)]" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">Transforming industries through technology, expertise and innovation.</p>
        </motion.div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center text-white">
                      ...
                      {domain.subtitle && <p className="text-gray-300 text-xs mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">{domain.subtitle}</p>}
                    </div>
                  </motion.div>
                );
              })}
        ...
        <div className="md:hidden">
          <div className="grid grid-cols-2 gap-4">
            {domains.slice(0, mobileVisibleCount).map((domain, index) => {
              ...
              return (
                ...
                >
                  <div className={`absolute inset-0 bg-gradient-to-br ${domain.color} opacity-20 transition-opacity duration-200`} />
                  <div className="absolute inset-0 bg-slate-800/50 backdrop-blur-sm transition-colors duration-200" />
          ...
          {mobileVisibleCount < domains.length && (
            <div className="mt-8 text-center">
              <motion.button onClick={handleLoadMore} className="bg-gradient-to-r from-blue-500 to-violet-600 text-white font-semibold py-3 px-8 rounded-full shadow-lg hover:shadow-xl transition-all duration-300" whileHover={{ scale: 1.03 }}>
                Load More...
              </motion.button>
            </div>
          )}
